	...
	public static List<String> removeComments(String[] source) {
        ...
        for(int i=0;i<source.length;i++){
            ...
            if(multi){
                ...
                if(ind>0 && ind!=source[i].length()-2){
                    res.add(source[i].substring(ind+2,source[i].length())); 
                }
                if(ind>0)
                	multi=false;
                continue;
            }
            int m=source[i].indexOf("/*");
            int s=source[i].indexOf("//");
            int e=source[i].indexOf("*/");
            ...
            if(m<0 && e<0 && s<0)
                ...
                if(s==0)
                	continue;
                if(s>0 && (s<m || m<0)){
                    sub=source[i].substring(0,s);
                }
                else{
                	if(m>=0){
                        sub=source[i].substring(0,m);
                        multi=true;
                    }
                }
                sb.append(sub);
                if(e>=0 && s<0){
                    sb.append(source[i].substring(e+2,source[i].length()));
                    multi=false;
                }
                if(sb.length()>0)
                	res.add(sb.toString());
